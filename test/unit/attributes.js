...
test( "non-lowercase boolean attribute getters should not crash", function() {
	...
	jQuery.each({
		...
		// The selector engine getter is called with the original casing
		try {
			strictEqual( jQuery.find.attr( elem[ 0 ], original ), lowercased,
				"The '" + this + "' selector engine getter should return the lowercased name" );
		} catch ( e ) {
			ok( false, "The '" + this + "' selector engine getter threw" );
		}

		// Attribute selectors reach the same getter when the native matcher is bypassed
		try {
			strictEqual(
				jQuery(
					"<input checked required autofocus type='checkbox'>" +
					"<input checked required autofocus type='checkbox'>"
				).filter( "[" + original + "]" ).length,
				2,
				"The '" + this + "' attribute selector should match both inputs" );
		} catch ( e ) {
